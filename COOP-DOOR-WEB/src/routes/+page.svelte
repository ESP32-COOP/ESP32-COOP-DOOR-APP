...
<!-- <button disabled type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center">
    ...
    Loading...
        ...
        <button disabled='{connect_text != "connect"?true:false }'  on:click={ connect} class="{connect_text != "connect"? "text-slate-400":"text-white"} bg-gradient-to-r from-slate-700 to-slate-900 rounded-3xl w-fit flex justify-center items-center h-20 text-white text-5xl uppercase px-3">
            {#if connect_text != "connect"}
            <svg aria-hidden="true" role="status" class="inline w-8 h-8 mr-2 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
