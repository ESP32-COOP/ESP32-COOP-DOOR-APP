...
export async function getDeviceInfo() {
	let options = {
		...
		optionalServices: [
			localBLE.serviceUUID,
			localBLE.dateCharUUID,
...
export function isDeviceConnected() {
	return (
		localBLE.device != undefined &&
		localBLE.device.gatt != undefined &&
		localBLE.device.gatt.connected
	);
}

export async function getDeviceGatt() {
	if (localBLE.device != undefined && localBLE.device.gatt != undefined)
		return await localBLE.device.gatt.connect();
	return false;
}

export function getDevice(): BluetoothDevice | undefined {
	return localBLE.device;
}
...
export async function readLight(): Promise<number[]> {
	if (localBLE.lightChar) {
		...
		return [
			Math.ceil(data[0] * divider),
			Math.ceil(data[1] * divider),
			Math.ceil(data[2] * divider)
		];
	}
	return [-1, -1, -1];
}
...
export async function writeDoor(turn: number, status: DoorMode) {
	if (localBLE.doorChar) {
		let buffer = new Uint8Array([turn * 10, status]).buffer;

		return await localBLE.doorChar.writeValue(buffer).catch((error) => {
			throw error;
		});
	}
}
...
export async function writeCloseDoor(mode: number, light: number, hour: number, minute: number) {
	if (localBLE.doorCloseChar) {
		let buffer = new Uint8Array([mode, Math.floor(light / 4), hour, minute]).buffer;
		await localBLE.doorCloseChar.writeValue(buffer);
	}
}

export async function writeOpenDoor(mode: number, light: number, hour: number, minute: number) {
